...
struct job_context {
    ...
    job_context(uchar* target, uchar* reference, uchar* img_out, int job_id):
        target(target), reference(reference), img_out(img_out), job_id(job_id) {}
};
...
public:
    ...
    __host__ __device__ bool is_full() {
        size_t tail = _tail.load(cuda::memory_order_acquire);
        size_t head = _head.load(cuda::memory_order_relaxed);
        return (tail-head) == N;
    }
...
__global__ void process_persistent_kernel(SPSC* cpu_to_gpu_queues, SPSC* gpu_to_cpu_queues, bool* running) {
    ...
    while (*running) {
        ...
        process_image(request.target, request.reference, request.img_out);
        __syncthreads();
...
public:
    ...
    bool enqueue(int job_id, uchar* target, uchar* reference, uchar* img_out) override {
        job_context new_job(target, reference, img_out, job_id);
